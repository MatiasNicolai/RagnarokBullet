...
- **Jefes** (layout transpuesto: poses en filas etiquetadas a la izquierda, 3 frames/columna): `tools/slice-bosses.mjs` — medir centros de fila de las etiquetas + rango x de frames por panel; extrae frame idle/attack. Cargar en `atlas.bosses` y el render de jefe (`renderer.syncBoss`) los usa vía `BOSS_TEX[nombre]`.
